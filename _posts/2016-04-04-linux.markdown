...
```
# cd到根目录
sudo find / -xdev -type f -printf '%s\t%p\n' 2>/dev/null   | sort -nr | head -n 50   | awk '{printf "%.2f GB\t%s\n", $1/1024/1024/1024, $2}'
```
...
# 测试下载速度
wget https://mirrors.tuna.tsinghua.edu.cn/ubuntu-releases/plucky/ubuntu-25.04-live-server-amd64.iso

# 指定下载目录
wget -P ./ --ftp-user=admin --ftp-password=pwd ftp://download.com
```
...
# 文件传输
rsync -avP ./source data02@192.168.1.16:/destination
```
...
在`Windows`中修改的`shell`脚本上传到`Linux`后出现乱码
```
